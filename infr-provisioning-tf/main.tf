...
/*resource "aws_key_pair" "ssh-key" {
  key_name   = "ansible-server-key-pair"
  public_key = file(var.ssh_key)
}*/

output "server-1-ip" {
    value = aws_instance.ansible-server-1.public_ip
}
output "server-2-ip" {
    value = aws_instance.ansible-server-2.public_ip
}
output "server-3-ip" {
    value = aws_instance.ansible-server-3.public_ip
}
...
resource "aws_instance" "ansible-server-3" {
  ami                         = data.aws_ami.amazon-linux-image.id
  instance_type               = var.instance_type
  key_name                    = "ansible-key"
  associate_public_ip_address = true
